...
export default function PartnerBrandsSection() {
  ...
  useEffect(() => {
    const fetchBrands = async () => {
      // ✅ دریافت دقیق ۷ برند از دیتابیس بر اساس order_index
      const { data, error } = await supabase
        .from('partner_brands')
        .select('id, name, logo_url, alt_text')
        .order('order_index', { ascending: true });
  ...
  return (
    <section 
      ...
      aria-label="برندهای معتبر همکار با فروشگاه چهره آپ"
    >
      <div className="container mx-auto px-4">
        {/* ✅ عنوان و توضیحات دقیقاً مطابق دیتابیس یا تنظیمات دلخواه */}
        <h2 className="text-2xl md:text-3xl font-bold text-gray-900 mb-3 text-center">
          برندهای معتبر همکار با فروشگاه چهره آپ
        </h2>
        <p className="text-gray-600 text-center mb-12 max-w-2xl mx-auto">
        ...
        {/* ✅ گرید ریسپانسیو برای نمایش ۷ برند */}
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-7 gap-6 items-center justify-items-center">
          {brands.map((brand) => (
            <div 
              key={brand.id}
            ...
            >
              <img
                src={brand.logo_url}
                // ✅ استفاده از alt_text اختصاصی که توی دیتابیس وارد کردی (عالی برای سئو)
                alt={brand.alt_text || `لوگوی برند ${brand.name}`}
                width={120}
                height={60}
                loading="lazy"
